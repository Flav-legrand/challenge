...
class DatabaseHelper {
  ...
  static Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onConfigure: (db) async {
        await db
            .execute('PRAGMA foreign_keys = ON'); // Activer les clés étrangères
      },
      onCreate: (db, version) async {
        ...
        await db.insert('matieres',
            {'id': 1, 'title': 'Math', 'icon': 'calculate', 'color': 'blue'});
        await db.insert('matieres', {
          'id': 2,
        ...
        await db.insert('matieres',
            {'id': 3, 'title': 'SVT', 'icon': 'grass', 'color': 'green'});
        await db.insert('matieres', {
          'id': 4,
          'title': 'Histoire & Géo',
          'icon': 'public',
        ...
        await db.insert('matieres',
            {'id': 7, 'title': 'Anglais', 'icon': 'language', 'color': 'red'});
        await db.insert('matieres', {
          'id': 8,
        ...
        await db.insert('matieres',
            {'id': 9, 'title': 'Français', 'icon': 'book', 'color': 'teal'});
        await db.insert('matieres',
            {'id': 10, 'title': 'Musique', 'icon': 'music', 'color': 'purple'});
        await db.insert('matieres',
            {'id': 11, 'title': 'Chimie', 'icon': 'flask', 'color': 'lime'});
        await db.insert('matieres', {
          'id': 12,
          'title': 'Sport',
          'icon': 'fitness_center',
        ...
        await db.insert('matieres',
            {'id': 14, 'title': 'Espagnol', 'icon': 'book', 'color': 'yellow'});

        // Insérer les données par défaut dans "devoirs"
        await db.insert('devoirs', {
          'id': 1,
          'titre': 'Devoir n°1',
          'statut': 'aVenir',
        ...
        await db.insert('verification', {
          ...
          'options':
              '["Produire des spermatozoïdes", "Produire de la progestérone", "Assurer la fécondation", "Produire des ovocytes"]',
        ...
        await db.insert('verification', {
          'id': 4,
          'question':
              'Comment appelle-t-on la fusion d’un spermatozoïde et d’un ovule ?',
        ...
        await db.insert('verification', {
          ...
          'options':
              '["Dans les trompes", "Dans l’utérus", "Dans les ovaires", "Dans les tubes séminifères"]',
  ...
  Future<List<Map<String, dynamic>>> getAllQuestionsPartI(
    ...
    return await db.query('verification'); // Ensure a valid return statement
  }
